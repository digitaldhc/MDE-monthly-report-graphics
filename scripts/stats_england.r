...
atp_stats_plot <- ggplot() + 
  ...
  # My Trust line
  geom_line(data = my_trust, aes(x = month, y = cyber_score, group = ods, colour = my_trust), colour = colour_my_trust, size = 3) +
  # top quartile line
  ...
  ggtitle(paste("Cyber exposure scores for endpoints - NHS Trusts -", first_month_print,"to", current_month_print, sep = " "), subtitle = "Data supplied by the NHS England Cyber Security Operations Centre") +
  # plot theme
  theme_base() +
  theme(
...
ggsave("atp_stats.png", width = 33.867, height = 19.05, units = "cm")
