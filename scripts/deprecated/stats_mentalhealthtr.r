# stats_mentalhealthtr.r
# Script to build NHSD monthly ATP reporting
# Mental Health Trusts
# Report deprecated 27/12/2024
...
# Mental health trust dataset less my Trust
subset_mhtr <- atp_stats_long_monthly %>%
  filter(ods == "RXM" | ods == "RT5" | ods == "RP7" | ods == "RP1" | ods == "RHA" | ods == "RT1" | ods == "R1L" | ods == "RWR" | ods == "RMY" | ods == "TAD" | ods == "RV9" | ods == "RGD" | ods == "RXE" | ods == "TAH" | ods == "RXG" | ods == "RW1" | ods == "RWX" | ods == "RNU" | ods == "RV3" | ods == "RKL" | ods == "RPG" | ods == "RV5" | ods == "RQY" | ods == "RRP" | ods == "TAF" | ods == "RWK" | ods == "RAT" | ods == "RNK" | ods == "RXY" | ods == "RXX" | ods == "RX2" | ods == "RX4" | ods == "RX3" | ods == "RXA" | ods == "RNN" | ods == "RXV" | ods == "RW5" | ods == "RW4" | ods == "RTV" | ods == "RT2" | ods == "RXT" | ods == "TAJ" | ods == "RYG" | ods == "RYK" | ods == "RLY" | ods == "RRE" | ods == "RTQ" | ods == "RVN" | ods == "RJ8" | ods == "RWV" | ods == "RH5")
...
atp_stats_plot <- ggplot() + 
  ...
  # background data
  geom_line(data = subset_mhtr, aes(x = month, y = cyber_score, group = ods, colour = ods), colour = alpha(colour_backgroundorgs, 0.7)) + 
  # average line
  ...
  # axis settings
  scale_x_date(date_labels = "%b %y", date_breaks = "3 months", expand = expansion(mult = c(0, .08))) +
  scale_y_continuous(breaks = c(20, 40, 60, 80, 100), limits = c(0,100)) +
  # axis labels
  xlab("Month") +
  ylab("Exposure score") +
  # plot title and subtitle
  ggtitle(paste("Cyber exposure scores for mental health Trusts only - January 2020 to", current_month_print, sep = " "), subtitle = "Data supplied by the NHS Digital Data Security Centre") +
  # plot theme
  theme_base() +
  theme(
...
ggsave("atp_stats_mhtr.png", width = 33.867, height = 19.05, units = "cm")
