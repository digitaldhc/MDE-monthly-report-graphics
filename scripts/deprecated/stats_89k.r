...
# NHS Trusts with between 8000-9999 EUC assets (list compiled manually)
...
# Similar Trust size less my Trust
subset_btr <- atp_stats_long_monthly %>%
  filter(ods == "R1K" | ods == "RA7" | ods == "RAL" | ods == "RAT" | ods == "RDU" | ods == "RDY" | ods == "RGT" | ods == "RHM" | ods == "RJ7" | ods == "RJE" | ods == "RM3" | ods == "RMC" | ods == "RTF" | ods == "RV3" | ods == "RW4" | ods == "RW5" | ods == "RW6" | ods == "RWK" | ods == "RWY" | ods == "RX3" | ods == "RX4" | ods == "RXK")
...
atp_stats_plot <- ggplot() + 
  ...
  ggtitle(paste("Cyber exposure scores for comparatively sized NHS Trusts - January 2020 to", current_month_print, sep = " "), subtitle = "Data supplied by the NHS Digital Data Security Centre") +
  # plot theme
  theme_base() +
  theme(
...
ggsave("atp_stats_largetr.png", width = 33.867, height = 19.05, units = "cm")
